...
var PatternModel = Backbone.Model.extend({
    ...
    getCardStrategy:function(){
        var index = this.get("index");
        var list = this.get("list")
        if ( index >= list )
            return null;
        var strategy = list[index]
        this.set("index",index+1)
        if ( strategy && strategy.type === "card" ) {
            if ( this.flipX  ) {
                strategy.start.x = cc.winSize.width - strategy.start.x;
                strategy.end.x = cc.winSize.width - strategy.end.x;
            }
            if ( this.flipY  ) {
                strategy.start.y = cc.winSize.height/2 + dimens.player1Y - strategy.start.y;
                strategy.end.y = cc.winSize.height/2 + dimens.player1Y - strategy.end.y;
            }
        }
        return strategy
    },
            ...
            if ( i % 2 == 0 ) {
                return {
                    ...
                    start: {
                        ...
                        x: cc.winSize.width + BORDER_PADDING + 1,
                        y: 150 + 40 * i
                    }
                }
            } else {
